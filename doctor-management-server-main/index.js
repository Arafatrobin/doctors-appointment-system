...
// const uri =
//   "mongodb+srv://newdb1:VeYM6ysHLQDfnhAk@cluster0.3pdbi.mongodb.net/doctorsMangement?retryWrites=true&w=majority";
const uri =
  "mongodb+srv://rakibul:aGQwafQK3DFh3ghE@cluster0.kt6ps.mongodb.net/doctorsMangement?retryWrites=true&w=majority";
...
client.connect((err) => {
  ...
  const categoryCollection = client
    .db("doctorsMangement")
    .collection("category");
  const bookTestCollection = client.db("doctorsMangement").collection("bookTest");

  app.post("/booktest", (req, res) => {
    bookTestCollection.insertOne(req.body).then((document) => {
  ...
  app.get("/booktest", (req, res) => {
    ...
    bookTestCollection.find(query).toArray((err, documents) => {
      res.send(documents);
    });
