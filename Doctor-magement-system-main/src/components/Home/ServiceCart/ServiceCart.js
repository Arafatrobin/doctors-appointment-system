...
const ServiceCart = () => {
  const fakeData = [
    ...
    },
    {
      title: abulance,
      descriptor:
        "Get ambulance Info by clicking Ambulance Service Button",
    ...
    },
    {
      title: blood,
      descriptor:
        "Now, you can get blood Donor in online! Just register or Login",
    ...
    <div className="row services">
      {fakeData.map((pd) => (
        <div className="col-md-6 col-lg-4 col-sm-12  main-service-cart">
          <div className="service-cart p-3 text-white">
            <img className="w-50" src={pd.title} alt="" />

            <p>{pd.descriptor}</p>
            <Link to={`${pd?.url}`}>
              <button className="btn btn-success service-cart-btn">{pd.buttonTitle}</button>
            </Link>
          </div>
        </div>
      ))}
